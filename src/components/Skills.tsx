import { Container, Typography, Box, Chip, Stack, Tooltip } from '@mui/material';
import { SiPython, SiJavascript, SiDjango, SiFastapi, SiReact, SiSpringboot, SiGit, SiDocker, SiMysql, SiRedis } from 'react-icons/si';
...
const skillGroups = [
  ...
  },
  {
    label: 'Architecture',
    skills: [
      { name: 'Microservices', icon: <SiReact color="#2cb67d" /> },
    ],
  },
  {
    label: 'Platforms',
...
const Skills: React.FC = () => {
  return (
    <Container sx={{ py: 6 }}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={4} justifyContent="center" alignItems="flex-start" sx={{ mt: 3 }}>
        {skillGroups.map((group, idx) => (
          <Box key={group.label} sx={{
            ...
            <Typography variant="subtitle1" fontWeight={700} sx={{ mb: 2, color: 'secondary.main', fontFamily: 'Space Grotesk, sans-serif', letterSpacing: 1 }}>
              {group.label}
            </Typography>
            <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap>
              {group.skills.map((skill, i) => (
                <Tooltip title={skill.name} key={skill.name} arrow>
                  ...
                  >
                    <Box sx={{ fontSize: 36, mb: 0.5, transition: 'color 0.3s' }}>{skill.icon}</Box>
                    <Typography variant="caption" sx={{ fontFamily: 'Space Grotesk, sans-serif', color: 'text.primary', fontWeight: 600 }}>
                      {skill.name}
                    </Typography>
                  </Box>
                </Tooltip>
